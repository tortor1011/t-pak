...
export default function BottomNavBar() {
  ...
  const hasBrokenNavigationApi = () => {
    return (
      typeof window !== 'undefined' &&
      'navigation' in window &&
      (window as Window & { navigation?: unknown }).navigation === null
    );
  };
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-8 pt-4 bg-white/80 backdrop-blur-md shadow-[0_-10px_40px_rgba(18,28,40,0.08)] rounded-t-3xl lg:hidden">
      {BOTTOM_NAV_ITEMS.map((item) => {
        ...
        const className = `flex flex-col items-center justify-center px-6 py-3 rounded-2xl active:scale-90 transition-all duration-200 ${
          active
